// src/pages/Bulk/components/H2so4/components/sourOption.ts
...
type BuildParams =
  ...
  | {
      ...
      legendPerRow?: number; // 新形态不再分行，但参数保留兼容
    };
...
function guessLineName(stacks?: StackSeries[]): string | undefined {
  ...
  // 优先匹配包含“总产量/周度/周产量”的
  const hit =
    stacks.find((s) => /总产量|周度|周产量/.test(s.name)) ?? stacks.find((s) => s.name.length >= 2);
  return hit?.name;
}

// 统一规整成 {dates,bars,lineName,lineValues,...}
function normalizeInput(input: BuildParams): {
  ...
  categoryGap: string | number;
  gridBg: string;
  legendPerRow: number; // 兼容保留（不再使用分行）
} {
  const common = {
...
export function buildSourOption(params: BuildParams): echarts.EChartsOption {
  ...
  return {
    ...
    xAxis: {
      ...
      axisLabel: { interval: labelInterval, color: '#7A8A99', margin: 10, rotate: 45, fontSize: 7 },
    },
    yAxis: {
      type: 'value',
      axisTick: { show: false },
